-- ============================================================================
-- SUPER ADVANCED CHAT PANEL UPGRADE - PART 2
-- Run this script SECOND (after Part 1 is successfully executed).
-- ============================================================================
...
-- ============================================================================
-- 2. INSERT TAX CHANNEL & SYNC USERS
-- ============================================================================
-- Insert the 'tax' channel if it doesn't exist
INSERT INTO chat_channels (name, channel_type, description)
SELECT 'Tax & Compliance', 'tax'::chat_channel_type, 'Tax preparation, audits, and compliance discussions'
WHERE NOT EXISTS (
  SELECT 1 FROM chat_channels WHERE channel_type = 'tax'::chat_channel_type
);

-- Sync existing tax users into the newly created tax channel
INSERT INTO chat_channel_members (channel_id, user_id)
SELECT c.id, p.id
FROM profiles p
CROSS JOIN chat_channels c
WHERE p.role = 'tax'::user_role 
  AND c.channel_type = 'tax'::chat_channel_type
ON CONFLICT (channel_id, user_id) DO NOTHING;

-- ============================================================================
-- 3. ADVANCED SEARCH RPC FOR MESSAGES (High Performance)
-- ============================================================================
